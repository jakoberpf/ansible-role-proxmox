  ...
  (1..N).each do |machine_id|
    config.vm.define "pve-#{machine_id}" do |machine|
      ...
      machine.vm.provider :virtualbox do |virtualbox|
        virtualbox.customize ['createmedium', 'disk', '--filename', "$PWD/pve-#{machine_id}_vdb.vdi", '--size', '128M', '--format', 'VDI']
        virtualbox.customize ['createmedium', 'disk', '--filename', "$PWD/pve-#{machine_id}_vdc.vdi", '--size', '128M', '--format', 'VDI']
